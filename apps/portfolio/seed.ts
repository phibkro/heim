/* eslint-disable @typescript-eslint/no-explicit-any -- seed script, slugs are auto-generated by hooks */
...
async function seed() {
	...
	for (const name of tagNames) {
		...
		if (existing.docs.length > 0) {
			...
			const tag = await payload.create({ collection: "tags", draft: false, data: { name } as any })
			tags[name] = tag.id
	...
	for (const project of projects) {
		...
		if (existing.docs.length > 0) {
			...
			await payload.create({ collection: "projects", draft: false, data: project as any })
	...
	const createdPosts: Record<string, number> = {}
	for (const post of posts) {
		...
		if (existing.docs.length > 0) {
			...
			const created = await payload.create({ collection: "posts", draft: false, data: post as any })
	...
	for (const entry of nowEntries) {
		...
		if (existing.docs.length > 0) {
			...
			await payload.create({ collection: "now-entries", draft: false, data: entry as any })
